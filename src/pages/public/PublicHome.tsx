import React, { Suspense } from 'react'
import HeroSection from '../../components/public/HeroSection'

const VideoSection = React.lazy(() => import('../../components/public/VideoSection'))
const FeaturesSection = React.lazy(() => import('../../components/public/FeaturesSection'))
const ScreenshotsSection = React.lazy(() => import('../../components/public/ScreenshotsSection'))
const ModulesSection = React.lazy(() => import('../../components/public/ModulesSection'))
const IntegrationSection = React.lazy(() => import('../../components/public/IntegrationSection'))
const ArchitectSection = React.lazy(() => import('../../components/public/ArchitectSection'))
const CtaSection = React.lazy(() => import('../../components/public/CtaSection'))

export default function PublicHome() {
  return (
    <>
      <HeroSection />
      <Suspense fallback={null}>
        <VideoSection />
        <FeaturesSection />
        <ScreenshotsSection />

        {/* TRUST STRIP */}
        <section className="py-12" style={{ background: 'var(--surface2)' }}>
          <div className="public-container text-center">
            <p className="text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: 'var(--text-tertiary)' }}>
              BEREITS LIVE & IN NUTZUNG
            </p>
            <div className="flex flex-wrap justify-center gap-8 md:gap-12 items-center">
              {[
                { name: 'Space AI', url: 'https://spaceai-henna.vercel.app' },
                { name: 'BauDoku AI', url: 'https://baudoku-ai.vercel.app' },
                { name: 'BuchBalance', url: '' },
                { name: 'Conser Shop', url: 'https://www.conser-gosch.de' },
              ].map(p => (
                <div key={p.name} className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full" style={{ background: '#22C55E' }} />
                  {p.url ? (
                    <a href={p.url} target="_blank" rel="noopener noreferrer"
                       className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                      {p.name}
                    </a>
                  ) : (
                    <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{p.name}</span>
                  )}
                </div>
              ))}
            </div>
            <p className="text-xs mt-4" style={{ color: 'var(--text-tertiary)' }}>
              4 Produkte live · 12,8 Mio. Produkte im Marktplatz · 7 Partner im Onboarding
            </p>
          </div>
        </section>

        <ModulesSection />
        <IntegrationSection />
        <ArchitectSection />
        <CtaSection />
      </Suspense>
    </>
  )
}
