    ...
    <div className="max-w-5xl mx-auto fade-up" style={{ height: 'calc(100dvh - var(--topbar-height) - var(--bottom-nav-total) - 2rem)' }}>
      ...
      <div className="flex flex-col md:grid md:grid-cols-3 gap-4 h-full">
        ...
        <div className="md:col-span-2 rounded-[20px] flex flex-col border overflow-hidden"
          style={{ background: 'var(--surface)', borderColor: 'var(--border)', minHeight: '420px', flex: '1 1 auto' }}>
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b shrink-0" style={{ borderColor: 'var(--border)' }}>
            ...
            {/* Mobile: Investitionsvorschlag Button */}
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2.5">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center py-10">
                ...
                <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Starten Sie jetzt das Gespräch</p>
              </div>
            )}
            {messages.map((m, idx) => {
        ...
        {/* ── Investment Panel (Desktop permanent, Mobile slide-in) ── */}
        <div className={`rounded-[20px] border flex-col gap-0 overflow-hidden shrink-0
          ...
          <div className="p-4 flex flex-col gap-3">
            {[
              { pct: '5%', label: 'Basispaket', desc: 'Einstiegsbeteiligung mit vollen Investor-Rechten', highlight: false },
              { pct: '10%', label: 'Premiumpaket', desc: 'Erweiterte Beteiligung mit Advisory-Position', highlight: true },
            ].map(opt => (
              <div key={opt.pct}
                className="rounded-[14px] p-4 border transition"
                style={{
                  background: opt.highlight ? 'rgba(6,61,62,0.06)' : 'var(--surface2)',
                  borderColor: opt.highlight ? 'rgba(6,61,62,0.2)' : 'var(--border)',
                }}>
                <div className="flex items-start justify-between mb-1.5">
                  <span className="text-xl font-bold" style={{ color: '#063D3E' }}>{opt.pct}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold"
                    style={{ background: opt.highlight ? '#063D3E' : 'var(--surface3)', color: opt.highlight ? 'white' : 'var(--text-secondary)' }}>
                    {opt.label}
                  </span>
                </div>
          ...
          <div className="p-4 pt-2 flex flex-col gap-3 mt-auto">
            ...
            <button
              ...
              className="w-full py-3 rounded-[12px] text-white font-semibold text-sm hover:opacity-90 disabled:opacity-35 transition flex items-center justify-center gap-2"
